...
\begin{document}
	...
	\begin{multicols*}{3}
		...
		\node [mybox] (box){%
			\begin{minipage}{0.3\textwidth}
			In a Markov decision process, a game agent interacts with an environment to achieve a certain goal. The interaction happens at every discrete time $ t = 1, 2, 3, ... $. The agent observes certain state of the environment $ S_t \in \mathcal{S} $, it selects some action $ A_t \in \mathcal{A} $ and then receive certain reward $ R_{t+1} \in \mathcal{R} $. 
			\end{minipage}
		};
		%------------ MDP Header ---------------------
